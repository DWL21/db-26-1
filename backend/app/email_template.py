...
<style>
  ...
  .badge-cat  { color:#888;border:1px solid #ddd; }
...
def _render_notice_card(notice: dict) -> str:
    ...
    category_html = f'<span class="badge badge-cat">{category}</span> ' if category else ""
    ...
    <tr>
      <td class="card-row">
        <table role="presentation" class="card-wrap" cellpadding="0" cellspacing="0">
          <tr>
            <td class="card-inner">
              <div class="card-meta">{status_html}{category_html}{dept_html}{date_html}</div>
...
def build_email_html(
    ...
    rows_html = "".join(_render_notice_card(n) for n in shown)
