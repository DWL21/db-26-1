...
// Sample notices per category — for the live mail preview.
    ...
    <div className="mailmock" aria-hidden="true">
      <div className="mailmock__head">
        <div className="mailmock__head-left">
          ...
          <div>
            <div className="mailmock__sender">숭실메일</div>
            <div className="mailmock__sender-sub">ssu-mail · 수신함</div>
          </div>
        </div>
        <div className="mailmock__time">{todayLabel()} · 09:00</div>
      </div>
      <div className="mailmock__body">
        <h3 className="mailmock__subject">
          오늘의 숭실대 공지 · 7건 (학사·장학·채용)
        </h3>
        ...
        <div>
          ...
          <div className="mailmock__item">
            ...
            <span className="mailmock__item-title">
              2026-1 수강신청 일정 안내
            </span>
          </div>
          <div className="mailmock__item">
  ...
  const steps = [
    {
      ...
      title: "관심 카테고리를 고릅니다",
      body:
        "학사·장학부터 채용·봉사까지 10개 카테고리 중 보고 싶은 것만 골라 담아요. 칩을 눌러 자유롭게 선택합니다.",
    },
    {
      n: "02",
      title: "수신할 이메일을 입력합니다",
      body:
        "받아볼 메일 주소를 알려주세요. 학교 메일도 개인 메일도 모두 가능합니다.",
    },
    {
      n: "03",
      title: "인증 후 구독 완료",
      body:
        "6자리 인증번호를 확인하면 끝. 매일 아침 9시, 선택한 카테고리의 새 공지가 도착합니다.",
    },
  ];
  return (
...
/* ───────── Subscribe flow ───────────────────────────────── */
function Subscribe() {
  const [step, setStep] = useState(1); // 1 categories, 2 email, 3 code, 4 done
  ...
  const selectAll = () => {
    setSelected(
      selected.length === CATEGORIES.length ? [] : CATEGORIES.map((c) => c.value)
    );
  };
  ...
  const goNext = () => {
    ...
    if (step === 1) {
      if (selected.length === 0) {
        setError("구독할 카테고리를 1개 이상 골라주세요.");
        return;
      }
      setStep(2);
    } else if (step === 2) {
      ...
      {items.map((it, i) => {
        ...
        return (
          <React.Fragment key={it.n}>
            <button className={`steps__item ${state}`} onClick={() => onJump(it.n)}>
              <span className="steps__num">
                {step > it.n ? "✓" : String(it.n).padStart(2, "0")}
              </span>
              {it.label}
            </button>
            {i < items.length - 1 && <span className="steps__bar" />}
          </React.Fragment>
        );
      })}
    </div>
...
function StepCategories({ selected, toggleCat, selectAll }) {
  return (
    <>
      ...
      <p className="form-card__sub">
        보고 싶은 카테고리를 골라주세요. 나중에 언제든지 바꿀 수 있어요.
      </p>
...
function StepEmail({ email, setEmail, selectedCount }) {
  return (
    <>
      <div className="form-card__head">
      ...
      <p className="form-card__sub">
        선택한 카테고리 {selectedCount}개의 새 공지를 매일 09:00 이 주소로 보내드립니다.
      </p>
      <div className="field">
...
function StepCode({ email, code, setCode, onEditEmail }) {
  const refs = useRef([]);
  ...
  return (
    <>
      ...
      <p className="form-card__sub">
        방금 발송된 6자리 인증번호를 입력해주세요. (최대 5분 소요)
      </p>
...
/* ───────── Live mail preview ────────────────────────────── */
function MailPreview({ selected, email }) {
  const items = useMemo(() => {
    return selected.flatMap((cat) =>
      (SAMPLE_NOTICES[cat] || []).map((n) => ({ ...n, cat }))
    );
  }, [selected]);
    ...
    <div className="mail">
      ...
      <div className="mail__body">
        <p className="mail__intro">
          {email ? <strong>{email.split("@")[0]}</strong> : "안녕하세요"}님, 오늘 새로 올라온 공지를 한 번에 정리했습니다.
        </p>
      ...
      <div className="mail__footer">
        숭실메일 (ssu-mail) — 숭실대학교 비공식 공지 메일링 · <a href="#">구독 해제</a> · <a href="#">설정 변경</a>
      </div>
    </div>
  );
}
...
function FAQ() {
  const qs = [
    {
      q: "유료인가요?",
      a: "아니요, 모두 무료입니다. 광고도, 트래킹 픽셀도 넣지 않습니다. 학내 공지를 더 쉽게 받아보자는 학생 사이드 프로젝트로 운영됩니다.",
    },
    {
      q: "공식 채널인가요?",
      a: "학교 공식 서비스는 아닙니다. 숭실대학교 홈페이지의 공개 공지를 크롤링해 정리해 보내드리며, 원문은 항상 메일 안의 링크에서 확인할 수 있습니다.",
    },
    {
      q: "구독을 해제하려면 어떻게 하나요?",
      a: "모든 메일 하단의 ‘구독 해제’ 링크 한 번이면 끝입니다. 별도 계정 가입이 없기 때문에 비밀번호도 필요하지 않습니다.",
    },
    {
      q: "어떤 정보를 저장하나요?",
      a: "메일 주소와 선택한 카테고리만 저장합니다. 학번이나 비밀번호는 저장하지 않으며, 발송 외 다른 용도로 사용하지 않습니다.",
    },
  ];
  return (
        ...
        <div className="foot__row">
          <div>
            <div className="foot__brand">
              숭실<span className="accent">메일</span>
            </div>
